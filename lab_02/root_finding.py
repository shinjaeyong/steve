...
def sequential(f, x0, delta_x=1e-6, epsilon=epsilon_global, b_verbose=False):
    """
    sequential method
    x0 로 부터 시작해서 delta_x 만큼씩 증가시키면서 lf(x)l 값이 epsilon 값 보다 작아지는지 관찰함
    :param f: f(x) = 0 인 x를 찾고자 하는 함수
    :param x0: x의 초기값
    :param delta_x: x를 한번에 delta_x 만큼씩 증가시킴
    :param epsilon_glabal: 오차 허용 범위
...
def newton(f, df, x0, epsilon=epsilon_global, b_verbose=False):
    ...
    xi ㅣㅈ점에서의 f(x)의 기울기가 di, 함숫값이 fi 이면
    ...
    while True:
        ...
        # 반복문이 한번 실해 되었으므로 counter 를 1 증가 시킴
        counter += 1
...
#end of func()
#BATzerk, "Square Root Calculator", Scratch, [Online} Available : https://scratch.nit.edu/projects/35488/
...
def main():
    ...
    x0 = "0.01" # x의 초기값. 정술인가? 실수인가? 문자열인가?
    ...
    #뉴튼-랩슨법 Newtion Raphson method 로 func() 의 해를 구하기 위해 시도
    # 초기값은 얼마인가?
